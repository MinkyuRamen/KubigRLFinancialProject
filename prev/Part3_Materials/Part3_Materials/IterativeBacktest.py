...
class IterativeBacktest(IterativeBase):
    ...
    def test_con_strategy(self, window = 1):
        ...
        # Contrarian strategy
        for bar in range(len(self.data)-1): # all bars (except the last bar)
            if self.data["rolling_returns"].iloc[bar] <= 0: #signal to go long >> MA가 (-)면 long position 가져야 함
                if self.position in [0, -1]:
                    self.go_long(bar, amount = "all") # go long with full amount
                    self.position = 1  # long position
            elif self.data["rolling_returns"].iloc[bar] > 0: #signal to go short >> MA가 (+)면 short position 가져야 함
                if self.position in [0, 1]:
                    self.go_short(bar, amount = "all") # go short with full amount
                    self.position = -1 # short position
        self.close_pos(bar+1) # close position at the last bar
    ...
    def test_boll_strategy(self, SMA, dev):
        ...
        for bar in range(len(self.data)-1): # all bars (except the last bar)
            if self.position == 0: # when neutral
                ...
                if self.data["price"].iloc[bar] > self.data["SMA"].iloc[bar]: # price가 SMA보다 위에 있을 때
                    if self.data["price"].iloc[bar] > self.data["Upper"].iloc[bar]: # signal to go short
                        # upper보다 price가 위에 있을 때 >> 매도 신호
                        self.go_short(bar, amount = "all") # go short with full amount
                        self.position = -1 # short position
                    else:
                        self.sell_instrument(bar, units = self.units) # go neutral, 중립
                        self.position = 0
            elif self.position == -1: # when short, 현재 short position
                if self.data["price"].iloc[bar] < self.data["SMA"].iloc[bar]: # price가 SMA 아래에 있으면
                    if self.data["price"].iloc[bar] < self.data["Lower"].iloc[bar]: # signal to go long
                        self.go_long(bar, amount = "all") # go long with full amount
                        self.position = 1 # long position
                    else:
                        self.buy_instrument(bar, units = -self.units) # go neutral, 중립
                        self.position = 0                
        self.close_pos(bar+1) # close position at the last bar, 포지션 청산
