...
class MLBacktester():
    ...
    def __init__(self, symbol, start, end, tc):
        ...
        self.end = end
        self.tc = tc
        self.model = LogisticRegression(C = 1e6, max_iter = 100000, multi_class = "ovr") # cls
        self.results = None
        self.get_data()
    ...
    def test_strategy(self, train_ratio = 0.7, lags = 5):
        ...
        split_index = int(len(full_data) * train_ratio) # train&test split 지점의 index
        ...
        # determine the number of trades in each bar => postion 변경 횟수
        self.data_subset["trades"] = self.data_subset["pred"].diff().fillna(0).abs()
        
        # subtract transaction/trading costs from pre-cost return => 거래비용으로 인한 손실도 고려
